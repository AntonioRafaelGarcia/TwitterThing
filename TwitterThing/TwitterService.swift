    ...
    class func getAuthUser(completion: (String?, User?) -> ()) {
        ...
        if let account = self.sharedService.account {
            ...
            request.performRequestWithHandler( { (data, response, error) -> Void in
                ...
                case 200...299:
                    do {
                        if let userData = try NSJSONSerialization.JSONObjectWithData(data, options: NSJSONReadingOptions.MutableContainers) as? [String : AnyObject] {
                            sharedService.user = TweetJSONParser.userFromData(userData)
                            completion(nil, sharedService.user); return
                        }
                ...
                case 400...499:
                    completion("ERROR: SLRequest type GET for /1.1/account/verify-credentials.json returned status code \(response.statusCode) [user input error].", nil)
                case 500...599:
                    completion("ERROR: SLRequest type GET for /1.1/account/verify-credentials.json returned status code \(response.statusCode) [server side error].", nil)
                default:
